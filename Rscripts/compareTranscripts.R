...
setwd("/Users/johanreimegard/Vetenskap/Data/deNovoExtension/spruce/info")

sample = "spruce.13_2.trinity" 

idxstats <- read.table("spruce_13_2_trinity_initial_spruce.13_2.trinity.round3.strict.sorted.idxstats",header=FALSE,sep="\t")
colnames(idxstats) <- c("Name","length","Mapped_reads","Unmapped_reads")

before <- read.table(paste(sample, "initial.ORFs.info", sep="."),header=FALSE,sep="\t")
colnames(before) <- c("Name","dir","ORFlength","SeqLength","start","stop","kind")
before$Name <- gsub(">","",before$Name)
before$Name <- gsub(" ","",before$Name)
before$kind2 = "ORF"
before$kind2[before$kind==1] = "5UTR"
before$kind2[before$kind==2] = "3UTR"
before$kind2[before$kind==3] = "FULL"
